...
class RxTAlpha(nn.Module, PyTorchModelHubMixin, pipeline_tag="text-generation", license="apache-2.0"):
    ...
    def _generate_single_token(
        ...
        outputs = self.forward(
            ...
            use_self_attn_cache=use_self_attn_cache, action=RxTForwardAction.DECODE, current_positions=torch.tensor([[input_ids.size(-1) - 1]]).to(input_ids.device) if not init_step else None
        )
